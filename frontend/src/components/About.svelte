    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
      ...
      <div class="relative">
        <div class="relative h-[500px] w-full rounded-2xl overflow-hidden shadow-2xl group">
          <img
            src={data?.image_url || 'https://images.unsplash.com/photo-1631624220291-8f191fbdb543?q=80&w=1080'}
        ...
        <div class="absolute -bottom-6 -right-6 w-48 h-48 bg-amber-500/10 border border-amber-500/30 rounded-2xl -z-10 hidden lg:block"></div>
      </div>
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
          <div class="p-4 bg-white/5 rounded-xl border border-white/10 hover:border-amber-500/50 transition-colors">
            <Award class="text-amber-400 mb-2" size={24} />
            <h3 class="font-semibold text-white">{data?.leadership_title || 'Liderazgo'}</h3>
            <p class="text-sm text-gray-400">{data?.leadership_desc || 'Formación de equipos...'}</p>
          </div>
